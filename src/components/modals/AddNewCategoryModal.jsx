...
const FoodIcon = () => (
  <svg width="40" height="40" viewBox="0 0 24 24" fill="currentColor" className="w-8 h-8">
      ...
      <div className="bg-white rounded-2xl max-w-2xl w-full">
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-100">
          ...
          <button
            ...
            className="w-10 h-10 flex items-center justify-center rounded-full bg-pink-500 hover:bg-pink-600 text-white text-xl font-medium"
          >
            ×
          </button>
        </div>
        ...
        <div className="p-6">
            ...
            <div className="mb-6">
              <div className="mb-6">
                ...
                <div className="flex flex-wrap gap-4 mb-6">
                  {subCategories['Entertainment & Attractions']?.map(({ name, icon }) => (
                    <button
                      key={name}
                ...
                <div className="flex flex-wrap gap-4">
                  {subCategories['Decoration & Styling']?.map(({ name, icon }) => (
                    <button
                      key={name}
                      onClick={() => handleSubCategoryClick(name)}
                      className={`relative px-6 py-3 rounded-full border-2 transition-all duration-200 flex items-center gap-3 ${
          ...
          <div className="flex justify-end gap-4 pt-6">
            <Button 
              onClick={handleCancel} 
              variant="secondary" 
              className="px-8 py-3 border border-gray-300 text-gray-700 rounded-2xl hover:bg-gray-50"
            >
              Cancel
            </Button>
            <Button 
              onClick={step === 1 ? handleNext : () => console.log('Selected:', selectedSubCategories)} 
              variant="primary" 
              className="px-8 py-3 bg-gradient-to-r from-pink-500 to-pink-600 text-white rounded-2xl hover:from-pink-600 hover:to-pink-700"
